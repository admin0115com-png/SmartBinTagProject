...
export default function App() {
      ...
      if (session?.user) {
        ...
        if (email) {
          ...
          const fallbackAvatar = avatar || 
                                 localStorage.getItem(`sbt_avatar_${email.toLowerCase().trim()}`) || 
                                 localStorage.getItem('sbt_last_selected_avatar') || 
                                 '';
          
          const res = mockDb.loginExternal(email, fName, lName, fallbackAvatar);
          if (res.success && res.user) {
            ...
            setProfilePhoto(res.user.profilePhoto || fallbackAvatar);
  ...
  const syncLocalDatabaseState = () => {
    ...
    if (user) {
      ...
      const fallbackPhoto = user.profilePhoto || 
                            ...
                            (user.email ? localStorage.getItem(`sbt_avatar_${user.email.toLowerCase().trim()}`) : null) || 
                            localStorage.getItem('sbt_last_selected_avatar') || 
                            '';
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const todayDayName = daysOfWeek[now.getDay()].toLowerCase();
      const tomorrowDayName = daysOfWeek[(now.getDay() + 1) % 7].toLowerCase();

      // Scan all bins registered to this account (fires whether a bin is selected or not)
      for (const bin of bins) {
        const beforeDateClean = (bin.beforeCollectionDate || '').toLowerCase().trim();
        const colDateClean = (bin.collectionDayDate || '').toLowerCase().trim();
        const nextColClean = (bin.nextCollection || '').toLowerCase().trim();

        const isBeforeDateMatch = 
          !bin.beforeCollectionDate ||
          bin.beforeCollectionDate === currentDateStrLocal ||
          bin.beforeCollectionDate === currentDateStrUTC ||
          beforeDateClean === todayDayName ||
          colDateClean === tomorrowDayName ||
          nextColClean.includes('tomorrow') ||
          nextColClean.includes(tomorrowDayName);

        const isDayDateMatch = 
          !bin.collectionDayDate ||
          bin.collectionDayDate === currentDateStrLocal ||
          bin.collectionDayDate === currentDateStrUTC ||
          colDateClean === todayDayName ||
          nextColClean.includes('today') ||
          nextColClean.includes(todayDayName);

        // 1. Before Collection Alarm Check
        if (bin.beforeCollectionEnabled && isBeforeDateMatch) {
          if (checkTimeMatch(bin.beforeCollectionTime, currentTimeStr12, currentTimeStr24)) {
            ...
            if (!dismissedAlarms.includes(alarmId) && !isSnoozed) {
              ...
              const body = `Attention! Put out your ${bin.colorName || bin.binType} ${bin.binType} bin for collection at ${bin.address || 'your address'}. Alert set for ${bin.beforeCollectionTime}.`;
        ...
        if (bin.collectionDayEnabled && isDayDateMatch) {
          if (checkTimeMatch(bin.collectionDayTime, currentTimeStr12, currentTimeStr24)) {
            ...
            if (!dismissedAlarms.includes(alarmId) && !isSnoozed) {
              ...
              const body = `Reminder: Today is collection day for your ${bin.colorName || bin.binType} ${bin.binType} bin at ${bin.address || 'your address'}. Collection time: ${bin.collectionDayTime}.`;
  ...
  const handleTriggerAlarm = (bin: Bin, alertType: 'before' | 'day' = 'day') => {
    const isBefore = alertType === 'before';
    const label = isBefore ? 'Evening Before Collection Alarm' : 'Collection Day Morning Alarm';
    const timeStr = isBefore ? (bin.beforeCollectionTime || '06:00 PM') : (bin.collectionDayTime || '07:00 AM');
    const tone = bin.alarmTone || 'Chime Classic';

    setActiveAlarm({
      serialNumber: bin.serialNumber,
      label: `${bin.binType.toUpperCase()} BIN (${label})`,
      tone,
      time: timeStr
    });
    setActiveAlarmBin(bin);
    setActiveAlarmType(isBefore ? 'Before Collection' : 'Collection Day');

    if (currentUser) {
      const loc = [bin.houseNumber, bin.street, bin.town, bin.postcode].filter(Boolean).join(', ') || 'Your Set Location';
      const title = `🚨 ${bin.binType.toUpperCase()} BIN COLLECTION ALERT: ${bin.serialNumber}`;
      const body = `Smart Bin Tag Collection Alert activated for ${bin.binType} Bin at ${loc}. Scheduled collection: ${bin.nextCollection || timeStr}.`;
      mockDb.addNotification(currentUser.uid, 'Collection Reminder', title, body, 'my-bins');
      setNotifications(mockDb.getNotifications(currentUser.uid));
    }
  };
  ...
  const handleAvatarUploadInSettings = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (dataUrl) {
        ...
        try {
          localStorage.setItem(`sbt_avatar_${currentUser.uid}`, dataUrl);
          if (currentUser.email) {
            localStorage.setItem(`sbt_avatar_${currentUser.email.toLowerCase().trim()}`, dataUrl);
          }
          localStorage.setItem('sbt_last_selected_avatar', dataUrl);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#04352b] pb-16 md:pb-0">
      ...
      <main className="flex-1">
        ...
        {/* Device Native Notification Permission Banner */}
        <DeviceNotificationPermissionBanner userId={currentUser?.id} />
